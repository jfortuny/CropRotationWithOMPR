...
library(dplyr)
s <-
  # Rglpk_read_file(file = 'c:/tmp/location.lp', 
  #                 type = 'CPLEX_LP')
Rglpk_read_file(file = 'c:/tmp/cplexFormattedLP.lp', 
                type = 'CPLEX_LP')
out <- Rglpk_solve_LP(
  obj = s$objective,
  mat = s$constraints[[1]],
...
attributes(s)
attributes(s)["objective_vars_names"]
attributes(s)$"objective_vars_names"
out$solution
sol <-
  data.frame(attributes(s)$"objective_vars_names", out$solution)
names(sol) <- c('Variable', 'Value')
sol
